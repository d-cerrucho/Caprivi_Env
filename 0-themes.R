...
g <- function(data){
  data %>% select(sort(names(.))) %>% glimpse
}

export_drive <- function(local_folder, drive_folder){
  
  all_images <- list.files(
    path = here::here(local_folder),
    pattern = "png", 
    all.files = TRUE, full.names = TRUE) 
  all_images %>% 
    map(~ googledrive::drive_upload(.,
                                    path = googledrive::as_dribble(drive_folder),
                                    overwrite = TRUE)
    )
}
# export_drive("plots_nutri", "juanchi_guille/JMF_fina_2021")
...
glimpsort <- function(x){ x %>% select(sort(names(x))) %>% glimpse }